...
const DAYS = Array.from({ length: 12 }, (_, i) => {
  const d = new Date(2026, 2, 7 + i);
  return { day: i + 1, date: d.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" }), city: i < 7 ? "Tokyo" : "Osaka" };
});
...
const INIT = {
  ...
  itinerary: DAYS.map((d,i)=>({...d,activities:SAMPLE_ACT[i]||[]})),
  hotels:[
    {id:"h1",city:"Tokyo",name:"Hotel Gracery Shinjuku",checkIn:"Mar 7",checkOut:"Mar 13",address:"1-19-1 Kabukicho, Shinjuku",conf:"GRC-20260307-4821",notes:"Godzilla head on the building!"},
    {id:"h2",city:"Osaka",name:"Cross Hotel Osaka",checkIn:"Mar 13",checkOut:"Mar 18",address:"2-5-15 Shinsaibashisuji, Chuo-ku",conf:"CRX-20260313-7193",notes:"Walking distance to Dotonbori."},
  ],
  flights:[
    {id:"f1",label:"Departure",airline:"ANA",flightNo:"NH 7",from:"SFO",to:"NRT",date:"Mar 7",time:"11:15 AM",conf:"ANA-8K2M4X"},
    {id:"f2",label:"Return",airline:"ANA",flightNo:"NH 178",from:"KIX",to:"SFO",date:"Mar 18",time:"2:30 PM",conf:"ANA-8K2M4X"},
  ],
  expenses:[
...
function Modal({open,onClose,title,children}){
  if(!open)return null;
  return(<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.35)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
    <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"22px 22px 0 0",maxWidth:430,width:"100%",maxHeight:"85vh",overflowY:"auto",padding:"20px 20px calc(20px + env(safe-area-inset-bottom))"}}>
      <div style={{width:36,height:4,background:"#DDD9D2",borderRadius:4,margin:"0 auto 16px"}}/>
      <div style={{fontFamily:"'Fraunces',serif",fontSize:18,fontWeight:700,marginBottom:18}}>{title}</div>
      {children}
    </div>
  </div>);
}
function Chip({city}){return<span style={{display:"inline-flex",padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600,letterSpacing:.3,background:city==="Tokyo"?"#E4F5EB":"#F0EAFF",color:city==="Tokyo"?"#1A7A52":"#7B4FC4"}}>{city}</span>}
function Btn({primary,ghost,danger,sm,full,children,style:s,...p}){
  const base={display:"inline-flex",alignItems:"center",gap:6,padding:sm?"7px 12px":"10px 18px",borderRadius:sm?10:12,border:"none",fontSize:sm?12.5:13.5,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",width:full?"100%":undefined,justifyContent:full?"center":undefined,...s};
  return<button style={primary?{...base,background:"#C84B31",color:"#fff"}:danger?{...base,background:"#FEE",color:"#D44"}:{...base,background:"#EDEBE6",color:"#605C55"}} {...p}>{children}</button>;
}
...
function ItineraryTab({data,save}){
  ...
  const[na,setNa]=useState({time:"",text:""});
  ...
  return(<div style={{padding:"12px 20px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8}}><Chip city="Tokyo"/><Chip city="Osaka"/></div>
    </div>
    {data.itinerary.map((day,i)=>{
      const open=exp===i;const acts=[...day.activities].sort((a,b)=>(a.time||"").localeCompare(b.time||""));const isTk=day.city==="Tokyo";
      return(<div key={i} style={{background:"#fff",borderRadius:16,border:"1px solid #DDD9D2",boxShadow:"0 1px 3px rgba(0,0,0,.04)",marginBottom:12,overflow:"hidden"}}>
        <div onClick={()=>setExp(open?-1:i)} style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:44,height:44,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:16,fontFamily:"'Fraunces',serif",background:isTk?"#E4F5EB":"#F0EAFF",color:isTk?"#1A7A52":"#7B4FC4"}}>{day.day}</div>
          ...
          {acts.length>0?(<div style={{position:"relative",paddingLeft:20}}>
            ...
            {acts.map((a,j)=>(<div key={a.id} style={{position:"relative",padding:"6px 0",display:"flex",gap:10,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <IconBtn danger onClick={e=>{e.stopPropagation();doRm(i,a.id)}} style={{width:28,height:28,marginLeft:8}}>{ic.trash}</IconBtn>
              </div>
            </div>))}
...
function BudgetTab({data,save}){
  ...
  return(<div style={{padding:"12px 20px"}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700}}>Budget</div>
      <Btn primary sm onClick={()=>setShowAdd(true)}>{ic.plus} Expense</Btn>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
      {[["Total Spent",`$${tot.toLocaleString("en",{maximumFractionDigits:0})}`,`¥${(tot*RATE).toLocaleString("en",{maximumFractionDigits:0})}`],["Per Person",`$${(tot/3).toLocaleString("en",{maximumFractionDigits:0})}`,`${data.expenses.length} expenses`]].map(([l,v,s],i)=>(
        <div key={i} style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:16,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
          <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:.5,color:"#9A958D",fontWeight:600}}>{l}</div>
          <div style={{fontFamily:"'Fraunces',serif",fontSize:24,fontWeight:700,marginTop:3}}>{v}</div>
          <div style={{fontSize:11.5,color:"#9A958D",marginTop:1}}>{s}</div>
        </div>
      ))}
    </div>
    ...
    {tab==="overview"&&<>
      {Object.keys(byCat).length>0&&<div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:16,marginBottom:12,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
        <div style={{fontSize:13,fontWeight:600,marginBottom:12}}>By Category</div>
        {Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([c,a])=>(<div key={c} style={{marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:2}}><span style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:8,height:8,borderRadius:4,background:catColor[c],display:"inline-block"}}/>{c}</span><span style={{fontWeight:600}}>${a.toFixed(0)}</span></div>
          <div style={{height:6,background:"#EDEBE6",borderRadius:6,overflow:"hidden"}}><div style={{height:"100%",borderRadius:6,width:`${(a/tot)*100}%`,background:catColor[c],transition:"width .4s"}}/></div>
        </div>))}
      </div>}
      <div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:"4px 16px",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
        {[...data.expenses].reverse().map(e=>(<div key={e.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 0",borderBottom:"1px solid #ECEAE5"}}>
          <div><div style={{fontSize:14,fontWeight:500}}>{e.description}</div><div style={{fontSize:12,color:"#9A958D"}}><span style={{width:6,height:6,borderRadius:3,background:catColor[e.category],display:"inline-block",marginRight:4}}/>{e.category} · {e.paidBy}</div></div>
          <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:600}}>${e.amount.toFixed(2)}</div><div style={{fontSize:11,color:"#9A958D"}}>¥{(e.amount*RATE).toLocaleString("en",{maximumFractionDigits:0})}</div></div><IconBtn danger onClick={()=>doRm(e.id)}>{ic.trash}</IconBtn></div>
        </div>))}
      </div>
    </>}
    {tab==="person"&&<div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:"4px 16px",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      {data.members.map(m=>(<div key={m} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"1px solid #ECEAE5"}}>
        <div><div style={{fontSize:14,fontWeight:600}}>{m}</div><div style={{fontSize:12,color:"#9A958D"}}>Paid ${data.expenses.filter(e=>e.paidBy===m).reduce((s,e)=>s+e.amount,0).toFixed(2)}</div></div>
        <div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:600}}>${(pps[m]||0).toFixed(2)}</div><div style={{fontSize:12,fontWeight:500,color:bal[m]>.01?"#1A7A52":bal[m]<-.01?"#D44":"#9A958D"}}>{bal[m]>.01?`Owed $${bal[m].toFixed(2)}`:bal[m]<-.01?`Owes $${(-bal[m]).toFixed(2)}`:"Settled ✓"}</div></div>
      </div>))}
    </div>}
    {tab==="settle"&&<div>{sett.length===0?<div style={{textAlign:"center",padding:32,color:"#9A958D"}}>All settled up!</div>:sett.map((s,i)=>(<div key={i} style={{background:"#FFF0EC",border:"1.5px solid #F0C4B6",borderRadius:16,padding:16,marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:500}}>{s.from}<span style={{color:"#C84B31",fontWeight:700,margin:"0 8px"}}>→</span>{s.to}</div>
      <div style={{textAlign:"center",fontFamily:"'Fraunces',serif",fontSize:24,fontWeight:700,marginTop:4,color:"#C84B31"}}>${s.amount.toFixed(2)}</div>
    </div>))}</div>}
...
function FoodTab({data,save}){
  ...
  const[ni,setNi]=useState({name:"",location:"",cuisine:"",notes:"",city:"Tokyo"});
  const doAdd=()=>{if(!ni.name.trim())return;const item={...ni,id:Date.now()+""};if(tab==="restaurants")save({...data,restaurants:[...data.restaurants,item]});else save({...data,quickEats:[...data.quickEats,item]});setNi({name:"",location:"",cuisine:"",notes:"",city:"Tokyo"});setShowAdd(false)};
  ...
  const Tab=({id,count,children})=><button onClick={()=>setTab(id)} style={{padding:"7px 14px",borderRadius:20,fontSize:13,fontWeight:500,border:"none",cursor:"pointer",fontFamily:"inherit",background:tab===id?"#17150F":"#EDEBE6",color:tab===id?"#fff":"#605C55"}}>{children}{count>0&&<span style={{marginLeft:4,fontSize:11,fontWeight:600,padding:"2px 6px",borderRadius:10,background:tab===id?"rgba(255,255,255,.2)":"#DDD9D2"}}>{count}</span>}</button>;
  ...
  return(<div style={{padding:"12px 20px"}}>
    ...
    <div style={{display:"flex",gap:6,marginBottom:14}}><Tab id="restaurants" count={data.restaurants.length}>Restaurants</Tab><Tab id="quickeats" count={data.quickEats.length}>Quick Eats</Tab></div>
    <div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:"4px 16px",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      {items.length===0?<div style={{textAlign:"center",padding:32,color:"#9A958D"}}>No items saved.</div>:items.map(item=>(<div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"1px solid #ECEAE5"}}>
        <div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}><span style={{fontSize:14,fontWeight:600}}>{item.name}</span><Chip city={item.city}/></div><div style={{fontSize:12.5,color:"#9A958D"}}>{[item.cuisine,item.location].filter(Boolean).join(" · ")}</div>{item.notes&&<div style={{fontSize:12.5,color:"#605C55",marginTop:3}}>{item.notes}</div>}</div>
        <IconBtn danger onClick={()=>doRm(item.id)} style={{marginLeft:8}}>{ic.trash}</IconBtn>
      </div>))}
    </div>
    <Modal open={showAdd} onClose={()=>setShowAdd(false)} title={tab==="restaurants"?"Add Restaurant":"Add Quick Eat"}>
      ...
      <div style={{display:"flex",gap:10}}><div style={{flex:1}}><Input label="Cuisine" placeholder="e.g. Ramen" value={ni.cuisine} onChange={e=>setNi({...ni,cuisine:e.target.value})}/></div><div style={{flex:1}}><Select label="City" value={ni.city} onChange={e=>setNi({...ni,city:e.target.value})}><option>Tokyo</option><option>Osaka</option></Select></div></div>
...
function HotelsTab({data,save}){
  ...
  return(<div style={{padding:"12px 20px"}}><div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>Hotels</div>
    {data.hotels.map(h=>(<div key={h.id} style={{background:"#fff",borderRadius:16,border:"1px solid #DDD9D2",padding:16,marginBottom:12,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><Chip city={h.city}/><span style={{fontSize:12,color:"#9A958D"}}>{h.checkIn} → {h.checkOut}</span></div>
...
function FlightsTab({data,save}){
  ...
  return(<div style={{padding:"12px 20px"}}><div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>Flights</div>
    {data.flights.map(f=>(<div key={f.id} style={{background:"#fff",borderRadius:16,border:"1px solid #DDD9D2",padding:16,marginBottom:12,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      <div style={{fontSize:12,fontWeight:700,color:"#C84B31",marginBottom:14,textTransform:"uppercase",letterSpacing:.8}}>{f.label} — {f.date}</div>
      ...
      <div style={{display:"flex",gap:10}}><div style={{flex:1}}><Input label="Time" value={f.time} onChange={e=>upd(f.id,"time",e.target.value)}/></div><div style={{flex:1}}><Input label="Confirmation" value={f.conf} onChange={e=>upd(f.id,"conf",e.target.value)}/></div></div>
    </div>))}
  </div>);
}
...
function ActivitiesTab({data,save}){
  const[showAdd,setShowAdd]=useState(false);const[na,setNa]=useState({name:"",city:"Tokyo",notes:"",cost:""});
  const doAdd=()=>{if(!na.name.trim())return;save({...data,activities:[...data.activities,{...na,id:Date.now()+""}]});setNa({name:"",city:"Tokyo",notes:"",cost:""});setShowAdd(false)};
  ...
  return(<div style={{padding:"12px 20px"}}>
    ...
    <div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:"4px 16px",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      {data.activities.length===0?<div style={{textAlign:"center",padding:32,color:"#9A958D"}}>No activities.</div>:data.activities.map(a=>(<div key={a.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"1px solid #ECEAE5"}}>
        <div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}><span style={{fontSize:14,fontWeight:600}}>{a.name}</span><Chip city={a.city}/></div><div style={{fontSize:12.5,color:"#9A958D"}}>{a.cost&&a.cost!=="0"?`~$${a.cost}/person`:"Free"}</div>{a.notes&&<div style={{fontSize:12.5,color:"#605C55",marginTop:3}}>{a.notes}</div>}</div>
        <IconBtn danger onClick={()=>doRm(a.id)} style={{marginLeft:8}}>{ic.trash}</IconBtn>
      </div>))}
    </div>
    <Modal open={showAdd} onClose={()=>setShowAdd(false)} title="Add Activity">
      <Input label="Activity Name" placeholder="e.g. teamLab Borderless" value={na.name} onChange={e=>setNa({...na,name:e.target.value})}/>
      <div style={{display:"flex",gap:10}}><div style={{flex:1}}><Select label="City" value={na.city} onChange={e=>setNa({...na,city:e.target.value})}><option>Tokyo</option><option>Osaka</option></Select></div><div style={{flex:1}}><Input label="Est. Cost (USD)" type="number" placeholder="—" value={na.cost} onChange={e=>setNa({...na,cost:e.target.value})}/></div></div>
...
function TransportTab(){
  const tips=[{t:"🚄 Shinkansen",d:"Tokyo → Osaka ~2.5hrs on Nozomi. Reserve seats in advance. Consider 7-day JR Pass."},{t:"🚇 IC Cards (Suica/ICOCA)",d:"Get Suica at Narita or any JR station. Works on trains, buses, konbini. Load ¥3,000–5,000."},{t:"📱 Google Maps",d:"Best transit nav for Japan. Real-time departures, platforms, costs. Download offline maps."},{t:"🚕 Taxis",d:"Clean, safe, expensive. Doors open automatically! Useful late night when trains stop ~midnight."},{t:"🚶 Walking",d:"Expect 15,000–25,000 steps/day. Wear comfy shoes!"},{t:"✈️ Tokyo → Osaka",d:"Shinkansen best (~$130/person). Alt: Peach/Jetstar flights (~$50 early)."}];
  return(<div style={{padding:"12px 20px"}}><div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>Getting Around</div>
    {tips.map((tip,i)=>(<div key={i} style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:16,marginBottom:10,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}><div style={{fontWeight:600,fontSize:14,marginBottom:5}}>{tip.t}</div><p style={{fontSize:13,color:"#605C55",lineHeight:1.6}}>{tip.d}</p></div>))}
  </div>);
}

// ── WEATHER ──
function WeatherTab(){return(<div style={{padding:"12px 20px"}}><div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>Weather</div>
  <div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:16,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
    <div style={{fontSize:15,fontWeight:600,marginBottom:8}}>March in Japan</div>
    <p style={{fontSize:13.5,color:"#605C55",lineHeight:1.65,marginBottom:18}}>Late winter/early spring. 40–55°F (5–13°C). Layers recommended! Pack a small umbrella.</p>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      <a href="https://weather.com/weather/tenday/l/Tokyo+Japan" target="_blank" rel="noopener" style={{fontSize:14,color:"#C84B31",textDecoration:"none",fontWeight:500}}>🗼 Tokyo 10-Day Forecast →</a>
      <a href="https://weather.com/weather/tenday/l/Osaka+Japan" target="_blank" rel="noopener" style={{fontSize:14,color:"#C84B31",textDecoration:"none",fontWeight:500}}>⛩️ Osaka 10-Day Forecast →</a>
    </div>
  </div>
</div>)}
...
function SettingsTab({data,save}){
  ...
  const colors=["#E4F5EB","#F0EAFF","#FFF0EC"];const tc=["#1A7A52","#7B4FC4","#C84B31"];
  return(<div style={{padding:"12px 20px"}}>
    <div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>Group Members</div>
    <div style={{background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,padding:"4px 16px",boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
      {data.members.map((m,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"1px solid #ECEAE5"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:18,background:colors[i],display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:tc[i]}}>{m.charAt(0)}</div><div style={{fontSize:14,fontWeight:500}}>{m}</div></div>
        <IconBtn onClick={()=>startEdit(i)}>{ic.edit}</IconBtn>
      </div>))}
    </div>
    <div style={{marginTop:28}}><div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>Danger Zone</div><Btn danger full onClick={()=>save(INIT)}>Reset All Data</Btn><p style={{fontSize:12,color:"#9A958D",textAlign:"center",marginTop:8}}>Resets to sample data.</p></div>
...
const moreItems=[
  ...
  {id:"settings",label:"Group & Settings",desc:"Members & data",bg:"#EDEBE6",ic2:"#605C55",icon:<svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>},
];
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // If no data in Firebase after 3s, seed it
    const timeout=setTimeout(()=>{if(!loaded){saveData(INIT);setLoaded(true)}},3000);
    return()=>{unsub();clearTimeout(timeout)};
  },[]);

  // Save to Firebase when data changes (skip if change came from Firebase)
  const save=(newData)=>{
    setData(newData);
    saveData(newData);
  };
  ...
  const render=()=>{
    switch(active){
      ...
      case"more":return(<div style={{padding:"12px 20px"}}>
        <div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:700,marginBottom:16}}>More</div>
        {moreItems.map(m=>(<div key={m.id} onClick={()=>setSub(m.id)} style={{display:"flex",alignItems:"center",gap:14,cursor:"pointer",padding:"14px 16px",background:"#fff",border:"1px solid #DDD9D2",borderRadius:16,marginBottom:10,boxShadow:"0 1px 3px rgba(0,0,0,.04)"}}>
          <div style={{width:42,height:42,borderRadius:13,background:m.bg,display:"flex",alignItems:"center",justifyContent:"center",color:m.ic2,flexShrink:0}}>{m.icon}</div>
          <div style={{flex:1}}><div style={{fontSize:14.5,fontWeight:600}}>{m.label}</div><div style={{fontSize:12.5,color:"#9A958D"}}>{m.desc}</div></div>
  ...
  return(<>
    ...
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",position:"relative",paddingBottom:90}}>
      <div style={{padding:"14px 20px 12px",position:"sticky",top:0,background:"rgba(245,244,240,.88)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",zIndex:100,borderBottom:"1px solid rgba(221,217,210,.5)"}}>
        ...
        :<><div style={{fontFamily:"'Fraunces',serif",fontSize:24,fontWeight:700,letterSpacing:-.4}}>Japan <span style={{color:"#C84B31"}}>'26</span></div><div style={{fontSize:12.5,color:"#9A958D",marginTop:1}}>Mar 7 – 18 · Tokyo & Osaka · 3 travelers</div></>}
      </div>
      ...
      <nav style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",maxWidth:430,width:"100%",background:"rgba(255,255,255,.96)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderTop:"1px solid rgba(221,217,210,.35)",display:"flex",justifyContent:"space-around",padding:"6px 12px calc(8px + env(safe-area-inset-bottom))",zIndex:100}}>
        {navItems.map(n=>{const isA=(tab===n.id&&!sub)||(sub&&n.id==="more");return(<button key={n.id} onClick={()=>{setTab(n.id);setSub(null)}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",padding:"8px 14px",cursor:"pointer",color:isA?"#C84B31":"#9A958D",fontSize:10.5,fontWeight:500,fontFamily:"inherit",borderRadius:12,position:"relative"}}>
          {isA&&<div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:20,height:2.5,background:"#C84B31",borderRadius:"0 0 4px 4px"}}/>}{n.icon}{n.label}
        </button>)})}
      </nav>
    </div>
  </>);
